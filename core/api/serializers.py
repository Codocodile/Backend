...
class SendInvitationRequestSerializer(serializers.ModelSerializer):
    ...
    def validate_group(self, data):
        ...
        try:
            ...
            raise serializers.ValidationError('No challenger with the id ({0}) found'.format(user.id))
        except Membership.DoesNotExist:
            raise serializers.ValidationError("You don't have any group.")
        if membership.role != "L":
            raise serializers.ValidationError("You are not Leader of the group.")
        if data["group"] != membership.group:
            raise serializers.ValidationError("You are not member of the group whit id ({0}).".format(data["group"].id))
        return data
